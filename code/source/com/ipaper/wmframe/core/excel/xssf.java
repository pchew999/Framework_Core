...

{
        ...
        throws ServiceException
	{
		...
		// readXLSX - reads a microsoft excel XLSX spreadsheet (.xlsx 2007 and newer)
		//
		...
		//
		//	Note that this and readXLS should be identical in every regard except for the references to hssf/xssf
		//
		...
		if (sPathfilename == null && fileStream == null) throw new ServiceException("readXLSX: 'pathfilename' or 'stream' required");
		if (sPathfilename != null && fileStream != null) throw new ServiceException("readXLSX: ambiguous input 'pathfilename' and 'stream' both supplied");
		...
		if (sheetNum == null) throw new ServiceException("readXLSX: input parameter 'sheetnum' required");
		if (cursor.first("useheaderrow")) useHeaderRow = (String) cursor.getValue();
		if (useHeaderRow == null) throw new ServiceException("readXLSX: input parameter 'useheaderrow' required");
		if (cursor.first("usesheetnames")) useSheetNames = (String) cursor.getValue();
		if (useSheetNames == null) throw new ServiceException("readXLSX: input parameter 'useSheetNames' required");
		if (cursor.first("loadAs")) loadAs = (String) cursor.getValue();
		if (loadAs == null) throw new ServiceException("readXLSX: input parameter 'loadAs' required");
		...
		try	{
			if (sPathfilename != null)	wb = new XSSFWorkbook(new FileInputStream(sPathfilename));
			if (fileStream != null)		wb = new XSSFWorkbook(fileStream);
			...
			if (sheethi + 1 > sheet_count) throw new ServiceException("readXLSX: input parameter 'sheetNum' exceeds number of available sheets");
				...
				for (int r = row_count + 13; r >= 0; r--) {		// why go beyond the 'end' of the worksheet?  i don't know, because people are idiots
					...
					if (row != null) {
						for (int c = 0; c <= col_count - 1; c++) {
							cell = row.getCell(c);
				        	if (cell != null && cell.getCellType() != XSSFCell.CELL_TYPE_BLANK) {		//CellType.BLANK
								maxr = r;
								break;
				        	}
						}
		...
		//IDataUtil.put( cursor, "maxr", ""+maxr );
				if (maxc == -1 || maxr == -1) throw new ServiceException("readXLSX: spreadsheet appears to be empty");
				...
				for (int c = 0; c <= maxc; c++) {
					...
					if(row != null) {
						...
						if (cell != null && useHeaderRow.equals("true")) {
							if (cell.getCellType() == XSSFCell.CELL_TYPE_STRING) col_name[c] = fix_name(cell.getStringCellValue());
							if (cell.getCellType() == XSSFCell.CELL_TYPE_NUMERIC) col_name[c] = fix_name(""+cell.getNumericCellValue());
						}
					}
						...
						for (int c = 0; c <= maxc; c++) {
							...
							if (cell != null) {
								switch (cell.getCellType()) {
									case XSSFCell.CELL_TYPE_STRING:			//CellType.STRING
										cval = cell.getStringCellValue();
										break;
		
									case XSSFCell.CELL_TYPE_NUMERIC:		//CellType.NUMERIC or CellType.BOOLEAN
										Double dval = cell.getNumericCellValue();
										if (isXSSFCellDateFormatted(cell)) {
											org.apache.poi.ss.usermodel.DataFormatter xdf = new org.apache.poi.ss.usermodel.DataFormatter();	//format it like excel does
											cval = xdf.formatCellValue(cell);
										}else{
									...
									case XSSFCell.CELL_TYPE_BLANK:			//CellType.BLANK
										cval = "";
										break;
	...
	public static boolean isXSSFCellDateFormatted(XSSFCell cell) {	//determine if the cell is a date (rather than just a number)
